...
(function () {
  ...
  if (calEl) {
    ...
    function updatePanel() {
      ...
      if (checkin && checkout) {
        req.href = '/prosser/versions/v0-1/contact?checkin=' + iso(checkin) + '&checkout=' + iso(checkout);
        var u = new URL(base);
        u.searchParams.set('check_in', iso(checkin)); u.searchParams.set('check_out', iso(checkout));
        ab.href = u.toString();
      } else {
        req.href = '/prosser/versions/v0-1/contact'; ab.href = base;
      }
    }
  ...
  if (form) {
    ...
    form.addEventListener('submit', function (e) {
      ...
      fetch('/api/request', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
        ...
        .catch(function (err) {
          statusEl.textContent = 'Something went wrong (' + err.message + '). Email us directly and we\'ll sort it out.';
