/* The Prosser Home — nav, gallery lightbox, availability calendar, request form. No dependencies. */
(function () {
  'use strict';
  ...
  if (calEl) {
    ...
    function updatePanel() {
      ...
      if (checkin && checkout) {
        req.href = '/prosser/versions/v0-5/contact?checkin=' + iso(checkin) + '&checkout=' + iso(checkout);
        var u = new URL(base);
        u.searchParams.set('check_in', iso(checkin)); u.searchParams.set('check_out', iso(checkout));
        ab.href = u.toString();
      } else {
        req.href = '/prosser/versions/v0-5/contact'; ab.href = base;
      }
    }
    ...
    Promise.resolve({ok:true,json:function(){return Promise.resolve({source:'demo',booked:[]});}})
      ...
      .then(function (data) {
        ...
        var names = { airbnb: 'Airbnb', vrbo: 'Vrbo', direct: 'direct bookings' };
        var list = (data.sources || []).map(function (s) { return names[s] || s; });
        var label = list.length > 1 ? list.slice(0, -1).join(', ') + ' and ' + list[list.length - 1] : list[0] || 'our booking';
        setStatus(data.source === 'live' || data.source === 'airbnb'
          ? 'Synced with ' + label + ' calendars' + (when ? ' · updated ' + when : '') + '.'
  ...
  /* ---- Pay page (PayPal.Me link with the quoted amount) ---- */
  var payBtn = document.getElementById('pay-button');
  if (payBtn) {
    var q = new URLSearchParams(location.search);
    var handle = (payBtn.dataset.paypal || '').replace(/^@/, '');
    var amount = (q.get('amount') || '').replace(/[^0-9.]/g, '');
    if (q.get('checkin')) document.getElementById('pay-in').textContent = fmt(fromIso(q.get('checkin')));
    if (q.get('checkout')) document.getElementById('pay-out').textContent = fmt(fromIso(q.get('checkout')));
    if (amount) document.getElementById('pay-amount').textContent = '$' + Number(amount).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' USD';
    if (handle) {
      payBtn.href = 'https://www.paypal.com/paypalme/' + encodeURIComponent(handle) + (amount ? '/' + amount + 'USD' : '');
    } else {
      payBtn.setAttribute('aria-disabled', 'true'); payBtn.classList.add('btn--ghost');
      payBtn.addEventListener('click', function (e) { e.preventDefault(); });
      document.getElementById('pay-status').textContent = 'Payment link coming soon. We\'ll include it in your confirmation email.';
    }
  }
